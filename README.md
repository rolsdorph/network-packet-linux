...
As if that wasn't annoying enough, it tests the packet socket functionality. In order to interact with `AF_PACKET` sockets, the test executable needs the `CAP_NET_RAW` capability - that is, it needs to *run as root*. It would be great to find a better way to deal with this, but I haven't gotten around to it yet, so for now I recommend building the tests as a normal user (`cabal build socketspec`) and running them as root (something like `sudo dist-newstyle/build/x86_64-linux/ghc-8.8.4/network-packet-linux-0.1.0.0/build/socketspec/socketspec`).
